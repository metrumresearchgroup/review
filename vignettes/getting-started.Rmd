...
contains a QC log. Its contents are shown below.
...
When a file is ready for its first QC, it should be assigned in the QC log. This
...
the updates should be checked in SVN. Multiple users are often completing QC of
...
completed on the file at revision 1. However, `headf = 4` indicates edits were
made to the file at revision 4, indicating another round of QC is needed.
...
To see the usefulness of this tool, run the following locally:
...
SVN, stratified by their QC status. Each contributor to repo has their own table.
